...
class Home(models.Model):
    ...
    descripcion = models.TextField(
        _("Descripcion"), default="I'm a professional content creator from Argentina"
    )
    imagen_fondo = models.ImageField(
        _("Img Fondo"), upload_to="home/", validators=[validate_image_size]
    )
...
def delete_old_imagen_fondo(sender, instance, **kwargs):
    if instance.pk:
        try:
            ...
            return
        if (
            old_instance.imagen_fondo
            and old_instance.imagen_fondo != instance.imagen_fondo
        ):
